...
with mp_pose.Pose(
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            if abs(right_hand.x - left_hand.x) < 0.2 and (not right_already_playing) and left_hand_ready:
                count_bings += 1
                print(f'{count_bings} triangle bings!')
                mid = MidiFile('music.mid', clip=True)
                msg = mido.Message('note_on', note=67, time=0.2)
                port.send(msg)
                time.sleep(msg.time)
                port.send(mido.Message('note_off', note=67))
                right_already_playing = True

            if abs(right_hand.x - left_hand.x) >= 0.2:
                right_already_playing = False
